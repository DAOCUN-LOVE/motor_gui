...
namespace Hardware {
    ...
    namespace DJIMotorManager {
        ...
        void unregister_motor(DJIMotor &motor) {
            std::unique_lock lock(data_lock);
            auto it = motors_map.find(motor.can_info.can_name_);
            if (it == motors_map.end()) {
                return;
            }

            auto &[can_, motors_] = it->second;
            motors_.erase(std::remove(motors_.begin(), motors_.end(), &motor), motors_.end());
            motor.motor_enabled_ = false;
            motor.give_current = 0;
            if (can_ != nullptr) {
                can_->unregister_callback_key(motor.can_info.callback_flag);
            }
        }
